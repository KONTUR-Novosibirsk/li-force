...
export default {
    ...
    methods: {
        ...
        store() {
            axios.post(route('admin.attribute.category.store'), this.categoryData)
            .then((response) => {
                ElNotification.success(
                    {
                        message: 'Категория добавлена', position: 'bottom-right'
                    })
        ...
        update() {
            axios.patch(route('admin.attribute.category.update', {
                shopCategoryAttribute: this.categoryData.id
            }), this.categoryData)
                .then((response) => {
